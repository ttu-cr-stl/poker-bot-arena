...
import type { SpectatorHandTimeline, SpectatorSeatState } from "../types";
...
const RANK_VALUE = RANK_SEQUENCE.split("").reduce<Record<string, number>>((acc, rank, index) => {
  acc[rank] = index + 2;
  return acc;
}, {});
...
function parseCardLabel(card?: string | null): ParsedCard | undefined {
...
function getStraightHigh(cards: ParsedCard[]): number | undefined {
...
function evaluateFive(cards: ParsedCard[]): HandRank {
...
function selectBestFive(cards: string[]): string[] {
  const parsed = cards.map((card) => parseCardLabel(card)).filter(Boolean) as ParsedCard[];
  if (parsed.length <= 5) {
...
function padCards(cards: (string | undefined)[]): (string | undefined)[] {
  const padded = [...cards];
...
function getWinnerCards(board: string[], seat?: SpectatorSeatState): (string | undefined)[] {
  if (seat) {
    const combined = [...(seat.hole ?? []), ...board].filter((label): label is string => Boolean(label));
...
export function HandTimeline({ hands, currentHandId, liveHandId, onSelect }: HandTimelineProps) {
  if (!hands.length) {
    return (
      <div className="timeline">
        <p className="timeline__empty">Hands will appear here once play begins.</p>
      </div>
    );
  }
  ...
  return (
    <div className="timeline">
      <h3 className="timeline__title">Hands</h3>
      <div className="timeline__list">
        {sorted.map((hand) => {
          ...
          const winnerResult = historyResults.reduce<(typeof historyResults)[number] | undefined>((best, result) => {
          ...
          return (
            <button
              ...
              type="button"
            >
              <div className="timeline__cards" aria-label={`Winning cards for ${winnerName}`}>
                {cardsToShow.map((card, index) => (
                  <Card key={`${hand.handId}-${index}`} card={card} size="xs" hidden={!card} />
                ))}
              </div>
              <div className="timeline__meta">
                <span className="timeline__winner-name">{winnerName}</span>
                <span className="timeline__pot">{formatMoney(potValue)}</span>
              </div>
            </button>
          );
        })}
